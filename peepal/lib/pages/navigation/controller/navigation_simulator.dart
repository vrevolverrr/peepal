...
class NavigationSimulator {
  ...
  void _prepareRoutePoints() {
    ...
    for (final step in directions) {
      ...
      _allPoints.addAll(PolygonUtil.decode(stepPolyline));
    }
  }
  ...
  void startSimulation() {
    ...
    _simulationTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_simulationStep >= _allPoints.length - 1) {
        ...
        onPositionUpdate(
            _createPosition(finalPoint.latitude, finalPoint.longitude));
      ...
      onPositionUpdate(
          _createPosition(currentPoint.latitude, currentPoint.longitude));
